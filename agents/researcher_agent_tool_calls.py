...
from utilities.langgraph_common_functions import (
    ask_human, after_ask_human_condition, bad_json_format_msg, multiple_jsons_msg, no_json_msg
)
import os
...
llm = ChatOpenAI(model="gpt-4o", temperature=0.2, timeout=120)
see_file = prepare_see_file_tool(work_dir)
tools = [see_file, final_response]
llm = llm.bind_tools(tools).with_config({"run_name": "Researcher"})
...
with open(f"{parent_dir}/prompts/researcher_tc_system.prompt", "r") as f:
...
class Researcher():
    def __init__(self, work_dir):
        ...
        researcher_workflow.add_node("tool", ToolNode(tools))
